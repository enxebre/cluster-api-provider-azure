...
// Like predicate.GenerationChangedPredicate
// but allows events when only status.lastUpdated is changed.
// This is the case when the node link controller notices that the associated node readiness has changed.
...
func (GenerationChangedPredicate) Update(e event.UpdateEvent) bool {
	...
	if e.MetaNew.GetGeneration() == e.MetaOld.GetGeneration() {
		var oldStatusClone *machinev1.MachineStatus
		var newStatusClone *machinev1.MachineStatus
		if _, ok := e.ObjectOld.(*machinev1.Machine); !ok {
			return false
		}
		oldStatusClone = e.ObjectOld.(*machinev1.Machine).Status.DeepCopy()
		if _, ok := e.ObjectNew.(*machinev1.Machine); !ok {
			return false
		}
		newStatusClone = e.ObjectNew.(*machinev1.Machine).Status.DeepCopy()
		// only LastUpdated has changed
		if derefTime(oldStatusClone.LastUpdated) != derefTime(newStatusClone.LastUpdated) {
			oldStatusClone.LastUpdated = nil
			newStatusClone.LastUpdated = nil
			if equality.Semantic.DeepEqual(oldStatusClone, newStatusClone) {
				return true
			}
		}
		return false
...
func derefTime(time *metav1.Time) metav1.Time {
	if time == nil {
		return metav1.Time{}
	}
	return *time
